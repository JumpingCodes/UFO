...
public class Asteroid{
    ...
    private Ufospiel dasUfospiel;

    private double DistanzX, DistanzY, Distanz;

    public Asteroid(Ufo pUfo, Schuss pSchuss, Ufospiel pUfospiel){
        kugel = new GLKugel(0,1000,0, 50,"Krater.jpg"); 

        dasUfo = pUfo;
        derSchuss = pSchuss;
        dasUfospiel = pUfospiel;
    ...
    public void bewegeDich(double s){
        ...
        if(kugel.gibY()<-700){
            this.kugel.setzePosition(x,1200,0);
            Ufospiel.AsteroidenPass++;
            System.out.println(Ufospiel.AsteroidenPass);
        }
        ...
        if (checkDistanzLazer()){
            this.kugel.setzeSichtbarkeit(false);
            Ufospiel.destroy(this);
            Schuss.SchussReady = true;
            Schuss.SchussAktiv = false;
            derSchuss.lock();
        }
    }
